...
download_model = 'ckpt.model'

# 通過 torch.cuda.is_available() 的回傳值進行判斷是否有使用 GPU 的環境，如果有的話 device 就設為 "cuda"，沒有的話就設為 "cpu"
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# 處理好各個 data 的路徑
# train_with_label = os.path.join(argv[1], 'training_label.txt')
# train_no_label = os.path.join(argv[2], 'training_nolabel.txt')
# testing_data = os.path.join(path_prefix, 'testing_data.txt')

testing_data_path = sys.argv[1]
output_path = sys.argv[2]
testing_data = os.path.join(path_prefix, testing_data_path)
...
model = torch.load(os.path.join(model_dir, download_model))
